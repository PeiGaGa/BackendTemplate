...
export default defineComponent({
  ...
  setup() {
    const store = useAppStore() // 使用 Pinia store
    ...
    const contentFullScreen = computed(() => store.contentFullScreen) // 使用 Pinia 状态
    ...
    function onFullscreen() {
      store.contentFullScreen = !store.contentFullScreen // 直接更新状态
    }
    ...
    function delMenu(menu, nextPath) {
      ...
      if (!menu.meta.hideClose) {
        if (menu.meta.cache && menu.name) {
          store.$patch({ keepAlive: { delKeepAliveComponentsName: menu.name } }) // 处理缓存
        }
      ...
      if (menu.path === activeMenu.value.path) {
        const prePage =
          index - 1 > 0 ? menuList.value[index - 1] : { path: defaultMenu.path }
    ...
    function setPosition() {
      if (scrollbarDom.value) {
        const domBox = {
          ...
          activeFather: scrollbarDom.value.wrapRef.querySelector(
            '.el-scrollbar__view',
          ),
        }
        let hasDoms = true
    ...
    function setKeepAliveData() {
      ...
      store.$patch({
        keepAlive: { setKeepAliveComponentsName: keepAliveNames },
      }) // 处理缓存
    }
